...
# Agent 评测：从 Trace 调试到可重复回归测试

Agent 失败时，团队常做两件事：翻一条日志改 Prompt，或者连续跑几次看“感觉有没有变好”。前者容易只修当前样例，后者无法区分随机波动和真实提升。
...
高敏感内容应脱敏或存入受控系统，Trace 里保留可关联 ID。日志完整不等于允许无限保存原始数据。

## 先定位失败发生在哪一层

同样是“报告错误”，可能来自完全不同的根因：
...
没有分层，团队容易用 Prompt 修一个本应由权限代码解决的问题。
...
如果文件夹、数据库快照、工具返回和策略版本没有固定，同一句输入每次面对的是不同任务，结果就无法比较。
...
优先使用确定性检查。只有语义质量难以机械判断时，再引入人工或 LLM Grader。
...
过程评分不是要求所有成功轨迹完全相同，而是检查关键不变量和明显浪费。
...
模型评分器容易被输出长度、语气和位置影响。使用前要准备人工标注样例，检查：
...
对关键安全和财务结果，不要让 LLM Grader 成为唯一裁判。

## Agent 有随机性，单跑一次不够

相同输入可能选择不同工具路径。评测时根据风险选择重复次数：
...
报告分布而不是只报最好一次：成功率、P50/P95 延迟、平均与尾部工具次数、费用分位数、失败簇占比。
...
防止数据集泄漏到 Prompt 或开发调试过程。团队如果知道全部标准答案，容易不自觉地过拟合。

## 版本比较要保留成本和风险角色

模型路由通常包含旗舰、平衡档和低成本模型。评测升级时不要把所有路径都换成最强模型，然后宣布质量上升。
...
不同任务角色可以选择不同模型，但路由规则本身也要进入评测版本。
...
用户满意度和任务完成率受流量结构影响；离线数据集又覆盖不了全部现实。两者要互相补充：
...
很多团队遇到失败就拆成多 Agent，结果协调成本上升，完成率没变。下一篇 [什么时候才需要多 Agent](/agents/advanced/multi-agent) 会把“拆分收益”放回同一套基线评测中判断。
