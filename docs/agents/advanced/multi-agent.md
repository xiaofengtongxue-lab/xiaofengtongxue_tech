...
“任务复杂，所以拆成多个 Agent”不是充分理由。复杂任务也可能通过更好的工具、结构化状态和确定性 Workflow 解决。多 Agent 值得存在的前提是：单 Agent 的失败能明确归因于需要隔离、并行或独立责任，而且评测证明收益大于协调成本。
...
<figure class="agent-diagram">
  ...
  <figcaption>先有单 Agent 基线和失败证据，再选择最小必要模式；不是先画组织架构，再找任务填进去。</figcaption>
</figure>
...
客服、财务和运维使用完全不同的规则和高风险工具。把全部说明塞给一个 Agent 会增加误选和权限暴露，可以先路由到专门 Agent。

### 独立任务可以并行

研究十家公司、检查一百个互不依赖文件，可以由 Orchestrator 分配给多个 Worker，再汇总结果。

### 需要独立候选和裁判

方案生成与质量评估需要不同指令、上下文或模型，Evaluator-Optimizer 才有意义。让同一个上下文里的两个角色互相附和，不一定获得独立性。

### 阶段责任天然不同

固定的检索、分析、合规检查和发布阶段由不同所有者维护，可以使用 Pipeline；其中只有需要动态判断的节点才用 Agent。
...
Handoff 时传递结构化任务摘要、来源和权限，不要把整个原始会话无差别复制给所有 Agent。
...
适合代码、翻译、报告等能清楚表达质量标准的任务。不适合“写得更好看”这类没有评分边界的无限润色。

Evaluator 应输出结构化缺陷和证据，而不是一句“还可以更完善”。循环同样需要最大轮次、无进展检测和成本上限。
...
阶段边界固定时，普通 Workflow 最容易测试和审计。每个节点都创建一个 Agent，只会增加随机性和状态同步。
...
没有合同，多 Agent 只是在自然语言里传话，问题发生后很难确定是谁丢了字段或扩大了权限。
...
如果多 Agent 只让架构图更丰富，却没有解决已知失败簇，就回到单 Agent。

## 框架功能和架构必要性不是一回事

某个 SDK 支持 Handoff、Subagent 或并行，不代表你的产品应该启用。供应商的多 Agent API、Beta Header 和 Item 类型会变化，先从业务合同和评测出发，再选择框架表达。
