...
Agent 永远只读的话，很安全，但做不了真正的工作。给它任意写入、发送和发布权限，又让人不放心。

解决办法不是二选一。而是把动作分级：低风险步骤自动执行，高风险副作用先生成可检查的候选，再由程序和人共同把关。
...
模型很适合判断"这份总结是否清楚"。但文件总数对不对、路径存不存在、章节齐不齐全——这些不需要再问一个模型。
...
if (countMatcher.find()
        && Integer.parseInt(countMatcher.group(1)) != files.size()) {
    issues.add("文件总数与真实目录不一致");
}
```
...
::: code-group
```bash [Java]
mvn -Dtest=ReportVerifierTest test
```
...
用户必须原样输入。这样能避免两件事：模型在报告正文里提前写一句"用户已确认"，以及脚本把普通的 `yes` 当成所有动作的通用授权。

光有随机令牌还不够。确认必须绑定当前草稿内容：
...
::: code-group
```java [Java]
String currentDigest = FileSupport.sha256(Files.readAllBytes(draftPath));
...
确认之后，如果 Agent、另一个进程或你自己改了草稿——旧授权立刻失效。
...
比如邮件审批，应该显示真实收件人、主题和附件，而不是只显示模型生成的"将发送一封合适的邮件"。参数变化后必须重新确认。

## 哪些动作默认应该停下来

不同业务风险不同，但下面几类通常不该只由模型自行决定：
...
"让用户确认"也不能代替服务端权限。即使用户输入了正确令牌，应用仍要确认他有权执行当前动作。

## 确认之后还要保证幂等

发布请求超时了怎么办？客户端不知道是否成功，再点一次可能产生重复副作用。

示例的本地文件替换天然比较简单。支付、发信和工单系统需要业务幂等键：
...
授权、执行和验收是三个不同阶段。把它们混在一起，出了问题你都不知道该查哪一个。
