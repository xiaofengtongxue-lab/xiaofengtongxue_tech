...
# 任务跑一半也能接着做：状态、上下文与检查点

资料盘点跑到第五轮时进程退出了。重新启动以后，Agent 应该从头再扫一遍，还是知道已经列过目录、搜索过 `TODO`，继续处理未完成步骤？这不是“模型记性好不好”，而是应用有没有保存任务状态。
...
在这个项目里，“已经搜索过 `TODO`”是任务状态；“用户习惯用中文报告”才可能是长期偏好。把前者错误地写进长期记忆，会让下一次新目录盘点继承旧现场。
...
- `next_step`：恢复后从哪一轮继续；
- `input_items`：保留 Tool Call、Tool Result 和必要模型 Item；
- `tool_events`：不用解析整段上下文，也能看到动作轨迹。
...
恢复时只保存“我正在搜索 TODO”这句摘要，会丢掉调用 ID、结构化参数和真实结果。对需要手动重放上下文的接口，Tool Call 与 Tool Result 必须保持配对，推理 Item 也要按官方要求保留。

自然语言摘要适合压缩旧过程，不适合代替关键协议状态。

## 检查点要原子写入

项目不会直接覆盖正式 JSON，而是先写临时文件，再替换：
...
如果进程正写到一半突然退出，旧检查点仍然完整。生产系统还要处理并发版本、数据库事务、对象存储一致性和损坏恢复，但“不要半写一个检查点”是共同起点。
...
尤其是付款、发信这类副作用，超时不代表没执行。恢复时应该先查询外部状态，不能直接再做一次。

## Context 不能无限增长

即使模型支持很长上下文，也不应该把所有旧工具日志永久塞进去。长上下文会带来：
...
怎么压缩、何时使用服务端 Conversation 或 Compaction，会在 [C 级上下文工程](/agents/advanced/context) 继续展开。

## 检查点本身也可能泄露数据

状态文件里可能出现用户输入、文件片段、工具结果和业务 ID。不要因为它位于 `.agent/` 就默认安全。生产环境需要明确：
...
示例把报告正文在 `tool_events` 中缩成字符数和 SHA-256，但 `input_items` 仍会保留工具协议所需内容。这是为了教学可见性，不是生产数据保留建议。
