---
title: AI Agent 评测入门
description: 为资料盘点 Agent 建立分层评测：先测工具和安全边界，再用固定任务集比较完成率、证据、工具轨迹、成本和人工接管。
...
# 从“这次成功”到“稳定成功”：给 Agent 建立评测

你运行示例目录，Agent 一次成功了。现在把 `meeting-notes.txt` 换成空目录、二进制文件、错误编码、带恶意指令的网页摘录，它还能完成吗？换模型、改 Prompt、增加工具以后，是更好了，还是只在演示样例上看起来更聪明？
...
这些测试不回答“模型会不会选对工具”，但能先排除 Harness 自己的确定性 Bug。边界代码都没测，就直接评模型，会把两类问题混在一起。
...
这些不需要 LLM Judge。只有“建议是否有用”“总结是否忠于证据”之类难以机械判断的质量，再考虑人工评分或模型评分，而且要用人工标注样例校准评分器。
...
能解释每一次失败属于模型、工具、状态、权限还是验证问题，你就不再只是“会运行 Agent Demo”，而是具备了构建可靠单 Agent 的基本方法。

## 接下来进入 C 级时会发生什么

B 级主线刻意把系统保持得很小：四个工具、本地 JSON 状态、单 Agent、人工终端审批。下一阶段不再增加“更炫的 Demo”，而是逐一回答工程问题：
