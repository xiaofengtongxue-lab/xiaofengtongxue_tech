...
# 让它连续做事：Agent Loop、停止条件与失败反馈

Tool Calling 解决了一次“模型提议、应用执行、结果回传”。可资料盘点至少需要列目录、搜索、读文件、写草稿四类动作。谁负责把这些调用一轮接一轮地串起来？答案就是 Agent Loop。
...
代码不长，难点都藏在“什么时候继续，什么时候必须停”。

## 模型停止输出，不代表任务完成

有些实现看到模型返回最终文本，就直接 `return response.output_text`。这相当于把完成标准交给模型自评。
...
所以模型没再调用工具时，程序先跑 `verify_report`。验收失败会把具体问题作为新反馈送回模型，让它修一次。只有验证器通过，状态才变成 `ready_for_approval`。

这就是“模型停止”和“任务完成”的区别。

## 最大轮次是最便宜的一道保险

模型可能不断搜索、不断改写，也可能因为工具结果不够清楚而犹豫。无论原因是什么，循环都不能无限运行。
...
`max_steps=10` 不是通用最佳值，只是这个小任务的本地上限。实际项目还应同时限制：
...
生产系统可以进一步判断“没有新增文件、没有新增证据、计划没有完成项变化”。签名只是最容易落地的起点。
...
它没有证明真实模型每次都能选对工具。那一层需要后面的行为评测，而不是单元测试代替。
...
- 示例循环使用手动输入 Item 管理状态，OpenAI SDK 版本为 2.48.0。
