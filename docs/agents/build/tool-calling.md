---
title: AI Agent Tool Calling 教程
...
# 给模型接上工具：Tool Calling 完整闭环

上一章里，模型真的找到了 `TODO`。问题来了：模型本身并不会在你的硬盘里执行 Python，它到底怎样“使用”了 `list_files`？

答案是 Tool Calling：**应用先把可用工具的名字、说明和参数结构告诉模型；模型返回一次结构化调用建议；应用校验并执行真实函数，再把结果绑定到原调用送回模型。**
...
如果应用没有提供读文件工具，模型最多根据文件名猜内容。Tool Calling 增加的是一条真实的数据通路，而不是一句更强势的提示词。

## 第一步：把 Python 函数翻译成模型能读懂的 Schema

打开 `file_audit_agent/tools.py`，`list_files` 的工具定义核心是：
...
这时候文件还没有被读取。它只是模型在说：“我建议现在调用 `list_files`，参数是 `.`。”

## 第三步：应用层决定能不能执行

示例项目没有写成 `globals()[name](**arguments)`，而是使用显式注册表：
...
目录越界时，模型拿到的不会是一段完整 Python 堆栈，而是：
...
错误契约能告诉模型：这是换参数才能解决的问题，不是无脑重试三次。
...
项目里的适配器会把 `response.output` 中的所有 Item 转成字典，追加到下一轮输入：
...
这才是下一篇要补上的 [Agent Loop](/agents/build/agent-loop)。
...
- 示例使用 OpenAI Python SDK 2.48.0 的 Responses API 结构；接口字段变化时以官方文档为准。
